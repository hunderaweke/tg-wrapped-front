...
const HeatMap = ({ data, type = 'daily' }) => {
  ...
  if (type === 'daily') {
    ...
    const daysDiff = Math.ceil((gridEndDate - gridStartDate) / (1000 * 60 * 60 * 24));
    ...
    for (let week = 0; week < weeks; week++) {
      ...
      const cellData = grid[0][week] || grid[1][week] || grid[2][week] || 
            ...
            <div className="day-labels">
              {dayLabels.map((day, idx) => (
                <div key={day} className="day-label">
                  {idx % 2 === 1 ? day.slice(0, 3) : ''} {/* Show only Mon, Wed, Fri */}
                </div>
              ))}
            </div>
                ...
                <div key={dayIdx} className="heatmap-row">
                  {week.map((cellData, weekIdx) => (
                    <div
                      ...
                      style={{
                        backgroundColor: cellData ? getColorIntensity(cellData.count) : 'var(--heatmap-level-0)'
                      }}
                      onMouseEnter={() => cellData && setHoveredCell(cellData)}
                      onMouseLeave={() => setHoveredCell(null)}
                    >
                      {hoveredCell === cellData && cellData && (
                        <div className="heatmap-tooltip github-tooltip">
                          <strong>{cellData.count} posts</strong>
                          <span>{cellData.month} {cellData.day}</span>
                        </div>
                      )}
                    </div>
                  ))}
